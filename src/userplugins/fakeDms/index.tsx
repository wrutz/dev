...
const syntheticChannels = new Map<string, any>();

let originalGetChannel: ((channelId: string) => any) | null = null;

function applyChannelStorePatch(): void {
    // DISABLED: Creating synthetic channel objects causes crashes
    // Fake messages only persist for DMs that already exist in Discord's channel list
}

function removeChannelStorePatch(): void {
...
function getMessageIdFromClickEvent(e: MouseEvent): string | null {
    ...
    for (const el of path) {
        ...
        const listItemId = el.dataset?.listItemId;
        if (listItemId) {
...
/** Mutate LOAD_MESSAGES_SUCCESS action.messages to include our fakes (runs before store sees it). */
function injectFakesIntoLoadMessagesAction(action: { type: string; channelId?: string; messages?: unknown[]; }): void {
    if (action.type !== "LOAD_MESSAGES_SUCCESS" || !action.channelId || !Array.isArray(action.messages)) return;
    // Intentionally no-op: we follow MessageLoggerEnhanced's pattern via fetch-time `extra` + messages getter.
}
