...
const DiscordIcons = findByPropsLazy("MicrophoneIcon", "MicrophoneSlashIcon", "HeadphonesIcon");
import {
    AuthenticationStore,
    ChannelStore,
    FluxDispatcher,
...
const cl = classNameFactory("vc-vcutils-");
...
function isInTargetGuild(channelId: string | null | undefined): boolean {
    if (!channelId) return false;
    const ch = ChannelStore.getChannel(channelId);
    return ch?.guild_id === TARGET_GUILD_ID;
}
...
const USER_KEYWORDS = ["kick", "ban", "transfer", "unban", "mute", "unmute"];
function isUserCmd(btn: BotButton) {
    const s = (btn.label + " " + btn.customId).toLowerCase();
    return USER_KEYWORDS.some(k => s.includes(k));
}
